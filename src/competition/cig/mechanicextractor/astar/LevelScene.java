package competition.cig.mechanicextractor.astar;

import java.util.ArrayList;
import java.util.List;

import com.sun.corba.se.impl.orbutil.graph.Graph;
import competition.cig.mechanicextractor.astar.level.Level;
import competition.cig.mechanicextractor.astar.level.SpriteTemplate;
import competition.cig.mechanicextractor.astar.sprites.BulletBill;
import competition.cig.mechanicextractor.astar.sprites.Enemy;
import competition.cig.mechanicextractor.astar.sprites.FireFlower;
import competition.cig.mechanicextractor.astar.sprites.Fireball;
import competition.cig.mechanicextractor.astar.sprites.FlowerEnemy;
import competition.cig.mechanicextractor.astar.sprites.Mario;
import competition.cig.mechanicextractor.astar.sprites.Mushroom;
import competition.cig.mechanicextractor.astar.sprites.Shell;
import competition.cig.mechanicextractor.astar.sprites.Sprite;
import competition.cig.mechanicextractor.astar.sprites.SpriteContext;
import competition.cig.mechanicextractor.extractor.*;
...
public class LevelScene implements SpriteContext, Cloneable
{
    ...
    public List<Sprite> getAddSprites() {
        return spritesToAdd;
    }

    public List<Sprite> getRemoveSprites() {
    ...
    @Override protected Object clone() throws CloneNotSupportedException 
    {
        ...
        for(Sprite item: this.sprites) 
        {
        	...
        	// since we see all these sprites, add them to the graph if they don't already exist in it
            Node extracted = new Entity(item, item.kind);
            if(!GraphBuilder.containsEntity(extracted)) {
                System.out.println("Adding new sprite: " + ((Entity) extracted).getType());
                GraphBuilder.addEntity(extracted);
            }
        }
        c.sprites = clone;
    	return c;
    ...
    public void bump(int x, int y, boolean canBreakBricks)
    {
        ...
        if ((Level.TILE_BEHAVIORS[block & 0xff] & Level.BIT_BUMPABLE) > 0)
        {
            ...
            if (((Level.TILE_BEHAVIORS[block & 0xff]) & Level.BIT_SPECIAL) > 0)
            {
                if (!mario.large)
                {
                    Sprite sprite = new Mushroom(this, x * 16 + 8, y * 16 + 8);
                    sprite.assignID();

                    addSprite(sprite);
                }
                else
                {   Sprite sprite = new FireFlower(this, x * 16 + 8, y * 16 + 8);
                    sprite.assignID();
                    addSprite(sprite);
                }
            }
