...
//added 2 variables to class and if-else statement at end of getAction()

public class OvercompensatingAgent implements Agent
{
	...
	OvercompensatingAgent errAgent;
        ...
        boolean[] ocompAction; //stores latest action and uses it as current action when buffer reaches certain #
    public void reset()
    {
    ...
    public boolean[] getAction(Environment observation)
    {
        ...
        if (ocompBuffer < 12) // number of actions between each overcompensation action
        {
            action = sim.optimise();
            ocompAction = action;
            ocompBuffer += 1;
